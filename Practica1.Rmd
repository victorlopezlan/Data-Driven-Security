...
- Mediante la librería 'stringr' impiamos el símbolo " que se encuentra al inicio de cada valor del campo Action y el símbolo " que se encuentra al final de cada valor del campo Protocolo.
...
colnames(epa_http) <- c("IP", "Timestamp", "Action", "File", "Protocolo", "ResponseCode", "Bytes")
...
#Limpiamos el simbolo " al inicio del campo Action con libreria stringr
epa_http$Action <- str_remove(epa_http$Action, '^"')
#Alternativa para limpiar " del inicio del campo Action
...
Justificación: De las 6539 peticiones que contienen ".edu" en el campo IP, filtramos de igual forma aquellas que tienen una extensión ".txt" en el campo File. Calculamos el total sumando las peticiones que cumplen con estas condiciones.
...
cat(sum(epa_http$File == "/"))
```

Justificación: Hemos calculado el total de filas donde el campo File contenga el símbolo /
